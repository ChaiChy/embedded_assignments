...
void USART2_IRQHandler (void)
{
	...
	if(message=='\r'){
		...
		//command = &USART2InputBuffer[USART2CmdFlag-5];
	
		if(USART2InputBuffer[0]=='s'){
			
			sscanf(USART2InputBuffer,"s%d", &winkel);		//Servo
			...
			sscanf(USART2InputBuffer,"r%dg%db%d", &r, &g ,&b);		//RGB-LED
		...
		*/
		...
		if(message=='\r'){
			...
			/*
			if(USART2InputBuffer[0]=='+' || USART2InputBuffer[0]=='-' ||USART2InputBuffer[0]=='0'){
				
				StepDir=USART2InputBuffer[0];
		}
			*/
